...
export function parseMorningEveningMonth(
  html: string,
  month: string,
  period: ReadingPeriod,
) {
...
export async function getMorningEveningEntry(
  ...
  const suffix = period === "morning" ? "AM" : "PM";
  const response = await fetch(`${SOURCE_BASE}/ME${month}${suffix}.html`, {
    next: { revalidate: 86400 },
  });

  if (!response.ok) {
    throw new Error("Unable to load Spurgeon's Morning and Evening text.");
  }
